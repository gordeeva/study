				...
				<tr>
					...
					<td>
						...
						<<option><c:out value="${role.name}" /></option>
							...
							</select> <input type="submit" value="<fmt:message key="ADD.BUTTON" />">
						</form>
						<form action="/webapp/EmployeeServlet" method="post">
						...
						<<option><c:out value="${role.name}" /></option>
							...
							</select> <input type="submit" value="<fmt:message key="DELETE.TABLE_HEADER" />">
						</form>

					</td>
					<td align="center"><a
						href="/webapp/EmployeeServlet?action=delete&id=<c:out value="${employee.id}"/>"><fmt:message key="DELETE.TABLE_HEADER" /></a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
